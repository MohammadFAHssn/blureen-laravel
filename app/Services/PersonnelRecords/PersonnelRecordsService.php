...
class PersonnelRecordsService
{
    ...
    public function getPersonnelRecords($data): array
    {
        ...
        $response ? $personnel_records['education_data'] = $response:throw new CustomException('خطا هنگام دریافت اطلاعات از سامانه جامع', 500);
        ...
        $response!==false ? $personnel_records['reassignment_data'] = $response:throw new CustomException('2خطا هنگام دریافت اطلاعات از سامانه جامع', 500);
        ...
        $response!==false ? $personnel_records['productivity_data'] = $response:throw new CustomException('خطا هنگام دریافت اطلاعات از سامانه بهره وری', 500);

        //assessment data
        $response = $this->fetchPersonnelRecordsFromSystems($personnelCode,config('services.payroll_system.get_assessment_data'));
        $response!==false ? $personnel_records['assessment_data'] = $response:throw new CustomException('خطا هنگام دریافت اطلاعات از سامانه فیش حقوقی', 500);

        //salary data
        $response = $this->fetchPersonnelRecordsFromSystems($personnelCode,config('services.payroll_system.get_payroll_data'));
        $response!==false ? $personnel_records['payroll_data'] = $response:throw new CustomException('خطا هنگام دریافت اطلاعات از سامانه فیش حقوقی', 500);

        //birthday gift data
        $response = $this->fetchPersonnelRecordsFromSystems($personnelCode,config('services.payroll_system.get_birthday_gift_data'));
        $response!==false ? $personnel_records['birthday_gift_data'] = $response:throw new CustomException('خطا هنگام دریافت اطلاعات از سامانه فیش حقوقی', 500);
        ...
        $response!==false ? $personnel_records['food_reserve_data'] = $response:throw new CustomException('خطا هنگام دریافت اطلاعات از سامانه رزرو غذا', 500);
